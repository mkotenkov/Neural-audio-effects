...
class CondWaveLearner(nn.Module):
    def __init__(self, n_channels, n_waves, cond_size, min_freq, max_freq, buffer_size):
        ...
        self.weights = nn.Parameter(torch.ones(n_channels, n_waves) / n_waves)

        self.condition_adaptor = nn.Sequential(
            nn.Linear(cond_size, 128), nn.Tanh(),
            nn.Linear(128, n_waves * 3)
    ...
    def forward(self, cond):
        ...
        w_mod, b_mod, f_mod = torch.chunk(self.condition_adaptor(cond), chunks=3, dim=1)

        weights = w_mod[:, None, :] + self.weights
        ...
        weights = weights.flatten(end_dim=1)[:, None, :]

        result = torch.bmm(weights, sines)
        result = result.squeeze()
        result = result.unflatten(0, [batch_size, self.n_channels])
        return result
